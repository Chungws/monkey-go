...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpHash:
			numKVs := int(code.ReadUint16(ins[ip+1:]))
			vm.currentFrame().ip += 2
...
func (vm *VM) executeMinusOperator() error {
	...
	operandValue := operand.(*object.Integer).Value
	return vm.push(&object.Integer{Value: -operandValue})
}
...
func (vm *VM) executeIndexExpression(left, index object.Object) error {
	...
	if leftType == object.ARRAY_OBJ && indexType == object.INTEGER_OBJ {
		return vm.executeArrayIndex(left, index)
	} else if leftType == object.HASH_OBJ {
